<template>
  ...
  >
    <div class="verification-pop">
      <div
        class="close-btn flex flex-item flex-justify"
      ...
      <div class="code-box row flex-justify-between flex-item">
        ...
        <input type="text" v-model="userValidCode" placeholder="请输入验证码" />
        <div class="get-code-btn">
          <div class="code-btn" v-if="!isGetCode" @click="handleGetCodeEvent">
            获取验证码
          </div>
          <div class="dis-code" v-else>{{ count }}s</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleSubmitEvent() {
      this.$toast("正在提交中");
      this.$emit("submitEvent", this.userValidCode);
    }
  }
};
</script>
